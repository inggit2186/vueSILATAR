                                    ...
                                    <table v-else class="table table-hover centered">
                                        ...
                                        <tbody>
                                            ...
                                            <tr v-else v-for="item in paginatedItem" :key="item.id">
                                                ...
                                                <td>
                                                    <BButton pill size="sm" variant="outline-primary" @click.prevent="changedetail(2,item.id)"><b><i-mdi-update /> UPDATE</b></BButton>
                                                </td>
                                            </tr>
                                        </tbody>
                                    </table>
    ...
    methods: {
        opener(){
            this.$swal.fire({
				...
				preConfirm: (nip) => {
                    ...
                    this.nip = nip
                    return this.$axios.post(import.meta.env.VITE_APP_API_URL+'/ceksatudata',{
                        nip: nip,
                    },{headers})
                    .then(response => {
        ...
        async addData(){
            try{
				...
				const response = await this.$axios.post(import.meta.env.VITE_APP_API_URL+'/updateSatuData',{
                    formx: this.kategori,
                    nip: this.nip
				}, {headers})
