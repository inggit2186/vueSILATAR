<template>
    <div class="main-wrapper">
        <usernavbar />
        <div class="page-wrapper laporan-pengaduan-page">
                                    ...
                                    <table class="table table-hover centered">
										...
										<tbody v-else-if="!loading && $route.params.id == 'admin'">
											...
											<tr v-for="(item,index) in paginatedItem" v-else :key="item.id" >
                                                ...
                                                <td @click.prevent="cetak(item.id, item.filename)"><b>{{ item.jam }}</b></td>
  ...
  methods: {
		async getError() {
			...
			try{
				...
				const response = await this.$axios.post(import.meta.env.VITE_APP_API_URL+'/myError',{
        ...
        async deleteAksi(id) {
            try{
				...
				const response = await this.$axios.post(import.meta.env.VITE_APP_API_URL+'/deleteError',{
					source: 'vuejs',
                    id : id
				},{headers})
		...
		async updateStatus(id,komen,st){
			...
			try{
				const headers = {
					...
					console.log(komen)
				const response = await this.$axios.post(import.meta.env.VITE_APP_API_URL+'/updateError',{
					id: id,
					komen: komen,
					status: st,
					bulan: this.xdate
